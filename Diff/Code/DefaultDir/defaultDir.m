...
% check if default file location exists
if isempty(fileLoc) == 0
    set(handles.fileloc,'String',fileLoc);   
else
    set(handles.fileloc,'String','Please set the default file location directory');
end
% check if default code location exists 
if isempty(mainLoc) == 0
    set(handles.diffmaintext,'String',mainLoc);   
else
    set(handles.diffmaintext,'String','Please set the code file directory');
end
% check if default output location exists
if isempty(opFile) == 0
    set(handles.FileOut,'String',opFile);   
else
    set(handles.FileOut,'String','Please set the default output location'); 
end
...
defaultSets = matfile('defaultdir.mat','Writable',true);
defaultSets.mainLoc = [];
defaultSets.mainLoc = defMainLoc;
set(handles.diffmaintext,'String',defMainLoc);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
opFile = handles.opFile;
defFileLoc = uigetdir(opFile,...
                   'Please select location of the default output directory');
